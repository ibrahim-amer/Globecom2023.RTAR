...
if (N >= M)
    for h=1:H
        for n = 1:N
            ...
            repmax_crq = RepMax_average_total_costs(n, 1, h) ./ rep_max_average_task_drop_rate(n, 1, h);
            rtar_crq = RTAR_average_total_costs(n, 1, h) ./ RTAR_average_task_drop_rate(n, 1, h);
            rtar_h_crq = RTAR_H_average_total_costs(n, 1, h) ./ RTAR_H_average_task_drop_rate(n, 1, h);
            repMax_plot = strcat(repMax_plot, '(', num2str(current_n),', ', num2str(repmax_crq), ',',num2str(hazard_rates(h)), ')');
            %rep_kw_plot = strcat(rep_kw_plot, '(', num2str(current_n),', ', num2str(rep_kw_average_task_drop_rate(n, 1, h)), ',',num2str(hazard_rates(h)),')');
            if (enable_true_benchmark)
                true_plot = strcat(true_plot, '(', num2str(current_n),', ', num2str(true_average_task_drop_rate(n, 1, h)), ',',num2str(hazard_rates(h)),')');
            end
            RTAR_plot = strcat(RTAR_plot, '(', num2str(current_n),', ', num2str(rtar_crq), ',',num2str(hazard_rates(h)),')');
            RTAR_H_plot = strcat(RTAR_H_plot, '(', num2str(current_n),', ', num2str(rtar_h_crq), ',',num2str(hazard_rates(h)),')');
        end
    end
else
    for h=1:H
