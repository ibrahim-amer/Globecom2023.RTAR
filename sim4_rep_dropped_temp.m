...
all_results = cell(1, length(0.1:0.1:1));
ctr = 1;
for ones_percentage=0.1:0.1:1
    %Workers history
    dataObj.workers_history_length = 100;
    dataObj.workers_history_percentageOfOnes_fromval = 0;
    dataObj.workers_history_percentageOfOnes_toval = ones_percentage;
    ...
    dataObj.worker_fitness_fn = @(rel_battery, worker_reputation, worker_cost, ... 
            task_data_size, task_processing_density) exp(rel_battery) ./ (worker_cost);
    dataObj.worker_fitness_fn_matlab = @(rel_battery, worker_reputation, worker_cost, ... 
        task_data_size, task_processing_density) -1 .* exp(rel_battery) ./ (worker_cost);

    all_results{ctr}.PBTA_rep_dropped_result = PBTA_simulation(N_min, N_max, N_stepSize, M_min, ... 
        M_max, M_stepSize, number_of_simulations, dataObj, checkConstraints);
...
PBTA_average_task_drop_rate = zeros(1, length(all_results));
PBTA_MCMF_average_task_drop_rate = zeros(1, length(all_results));
PBTA_rep_dropped_average_task_drop_rate = zeros(1, length(all_results));
PBTA_MCMF_rep_dropped_average_task_drop_rate = zeros(1, length(all_results));

for i=1:length(all_results)
    try 
        current_m = all_results{i}.PBTA_result{1,1}.dataObj.M;
        current_n = all_results{i}.PBTA_result{1,1}.dataObj.N;
        current_dataObj = all_results{i}.PBTA_result{1, 1}.dataObj;
        current_dataObj_rep_dropped = all_results{i}.PBTA_rep_dropped_result{1, 1}.dataObj;


        %% Step 1: prepare decision vars
        %PBTA
        PBTA_x = all_results{i}.PBTA_result{1, 1}.all_sims{1}.optimal_solution.x;
        ...
        %PBTA rep dropped
        PBTA_rep_dropped_x = all_results{i}.PBTA_rep_dropped_result{1, 1}.all_sims{1}.optimal_solution.x;
        ...
        %PBTA MCMF
        PBTA_MCMF_x = double(all_results{i}.PBTA_result{1, 1}.all_sims{1}.MCMF_result.X);
        ...
        PBTA_MCMF_rep_dropped_x = double(all_results{i}.PBTA_rep_dropped_result{1, 1}.all_sims{1}.MCMF_result.X);
        ...
        PBTA_average_task_drop_rate(i) = PBTA_average_task_drop_rate(i) + PBTA_percentage;
        ...
        PBTA_MCMF_average_task_drop_rate(i) = PBTA_MCMF_average_task_drop_rate(i) + PBTA_MCMF_percentage;
        ...
        PBTA_rep_dropped_average_task_drop_rate(i) = PBTA_rep_dropped_average_task_drop_rate(i) + PBTA_rep_dropped_percentage;
        ...
        PBTA_MCMF_rep_dropped_average_task_drop_rate(i) = PBTA_MCMF_rep_dropped_average_task_drop_rate(i) + PBTA_MCMF_rep_dropped_percentage;
...
ones_percentage = 0.1:0.1:1;
for i=1:length(all_results)
    %% Step 4: build graphs
    current_m = 50;
    current_n = 100;
    current_one_percent = ones_percentage(i);
    PBTA_plot = strcat(PBTA_plot, '(', num2str(current_one_percent),', ', num2str(PBTA_average_task_drop_rate(i)), ')');
    PBTA_MCMF_plot = strcat(PBTA_MCMF_plot, '(', num2str(current_one_percent),', ', num2str(PBTA_MCMF_average_task_drop_rate(i)), ')');
    ...
    PBTA_rep_dropped_plot = strcat(PBTA_rep_dropped_plot, '(', num2str(current_one_percent),', ', num2str(PBTA_rep_dropped_average_task_drop_rate(i)), ')');
    PBTA_MCMF_rep_dropped_plot = strcat(PBTA_MCMF_rep_dropped_plot, '(', num2str(current_one_percent),', ', num2str(PBTA_MCMF_rep_dropped_average_task_drop_rate(i)), ')');
end
