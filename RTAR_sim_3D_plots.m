        ...
        for n = 1:N
            for m=1:M
                try 
                    ...
                    RepMax_average_num_replicas(n, m, h) = RepMax_average_num_replicas(n, m, h) + sum(rep_max_replicas);
                    ...
                    RTAR_average_num_replicas(n, m, h) = RTAR_average_num_replicas(n, m, h) + sum(RTAR_replicas);
                    ...
                    RTAR_H_average_num_replicas(n, m, h) = RTAR_H_average_num_replicas(n, m, h) + sum(RTAR_H_replicas);
                    ...
                    rep_max_average_task_drop_rate(n, m, h) = rep_max_average_task_drop_rate(n, m, h) + rep_max_percentage;
...
%CRQ
repMax3D_plot2.x = zeros(H, M);
repMax3D_plot2.y = zeros(H, M);
repMax3D_plot2.z = zeros(H, M);

rtar3D_plot2.x = zeros(H, M);
rtar3D_plot2.y = zeros(H, M);
rtar3D_plot2.z = zeros(H, M);

rtarH3D_plot2.x = zeros(H, M);
rtarH3D_plot2.y = zeros(H, M);
rtarH3D_plot2.z = zeros(H, M);
...
for h=1:H
    for m = 1:M
        ...
        %CRQ
        repMax3D_plot2.x(h, m) = current_m;
        repMax3D_plot2.y(h, m) = hazard_rates(h);
        repMax3D_plot2.z(h, m) = (RepMax_average_total_costs(1, m, h) ./ current_m) ./ rep_max_average_task_drop_rate(1, m, h);

        rtar3D_plot2.x(h, m) = current_m;
        rtar3D_plot2.y(h, m) = hazard_rates(h);
        rtar3D_plot2.z(h, m) = (RTAR_average_total_costs(1, m, h) ./ current_m) ./ RTAR_average_task_drop_rate(1, m, h);

        rtarH3D_plot2.x(h, m) = current_m;
        rtarH3D_plot2.y(h, m) = hazard_rates(h);
        rtarH3D_plot2.z(h, m) = (RTAR_H_average_total_costs(1, m, h) ./ current_m) ./ RTAR_H_average_task_drop_rate(1, m, h);
...
p1 = plot3(x1, y1, z1, '-^', 'linewidth', 2, 'Color', '#00D327', 'DisplayName', 'RTAR');
hold on;
...
p2 = plot3(x2, y2, z2, '-.x', 'linewidth', 2, 'Color', '#2343C5', 'DisplayName', 'RTAR-H');
...
p3 = plot3(x3, y3, z3, '-.x', 'linewidth', 2, 'Color', '#FF0000', 'DisplayName', 'RMEE');

xlabel('Task Assignment Load Ratio');
ylabel('Failure Rates');
zlabel('Task Drop Rate');
title('Task assignment load ratio to task drop rate - plot3');


% % Concatenate the plot handles
all_plots = [p1(1); p2(1); p3(1)];

% Create legend
legend(all_plots, 'RTAR', 'RTAR-H', 'RMEE');
...
% Plot the surfaces
hold on;
surf(x1, y1, z1, 'FaceColor', 'green'); % Surface 1 with red color

surf(x2, y2, z2, 'FaceColor', 'blue'); % Surface 2 with green color
surf(x3, y3, z3, 'FaceColor', 'red'); % Surface 3 with blue color

% Set labels and title
xlabel('Task Assignment Load Ratio');
ylabel('Failure Rates');
zlabel('Task Drop Rate');
title('Task assignment load ratio to task drop rate - 3D Surface');


% Adjust the view angle for better visibility
view(45, 30);

% Create dummy plots with desired colors for the legend
dummy1 = plot(NaN, NaN, 's', 'MarkerFaceColor', 'red', 'MarkerEdgeColor', 'none');
dummy2 = plot(NaN, NaN, 's', 'MarkerFaceColor', 'green', 'MarkerEdgeColor', 'none');
dummy3 = plot(NaN, NaN, 's', 'MarkerFaceColor', 'blue', 'MarkerEdgeColor', 'none');

% Create legend
legend([dummy1, dummy2, dummy3], 'RTAR', 'RTAR-H', 'RMEE');
