%% This simulation compares PBTA/MCMF and PBTA/MCMF where reliability and reputation constraints are ignored
%Preparing the data
N_min = 100;
N_max = 100;
N_stepSize = 5;
...
task_data_size, task_processing_density) 1 ./ (worker_cost .* task_data_size .* task_processing_density);

dataObj.run_MCMF = true;
sim.PBTA_result_rel_rep_dropped = PBTA_simulation(N_min, N_max, N_stepSize, M_min, ... 
    M_max, M_stepSize, number_of_simulations, dataObj, checkConstraints);
...
PBTA_plot = '';
PBTA_MCMF_plot = '';
PBTA_MCMF_rel_rep_dropped_plot = '';
PBTA_rel_rep_dropped_plot = '';
...
PBTA_MCMF_rel_rep_dropped_percentages_average = zeros(N, M);
PBTA_rel_rep_dropped_percentages_average = zeros(N, M);

for n=1:N
    for m=1:M       
        PBTA_percentages_average(n, m) = double(0);
        PBTA_MCMF_percentages_average(n, m) = double(0);
        PBTA_MCMF_rel_rep_dropped_percentages_average(n, m) = double(0);
        PBTA_rel_rep_dropped_percentages_average(n, m) = double(0);
        for sim_count=1:number_of_simulations
            try 
                ...
                %PBTA MCMF reliability and reputation dropped
                PBTA_MCMF_rel_rep_dropped_x = double(sim.PBTA_result_rel_rep_dropped{n, m}.MCMF_result.X);
                PBTA_MCMF_rel_rep_dropped_x_reshaped = reshape(PBTA_MCMF_rel_rep_dropped_x, [current_m, current_n]);
                PBTA_MCMF_rel_rep_dropped_replicas = sum(PBTA_MCMF_rel_rep_dropped_x_reshaped, 2);
                %PBTA relibaility and repuation constraints dropped
                PBTA_rel_rep_dropped_x = sim.PBTA_result_rel_rep_dropped{n, m}.all_sims{sim_count}.x;
                PBTA_rel_rep_dropped_x_reshaped = reshape(PBTA_rel_rep_dropped_x, [current_m, current_n]);
                PBTA_rel_rep_dropped_replicas = sum(PBTA_rel_rep_dropped_x_reshaped, 2);


                %% Step2: Calculate failure probability
                failure_rel_probs = 1 - sim.PBTA_result{n,m}.dataObj.workers_tasks_rel_prop;
                failure_rel_probs = reshape(failure_rel_probs, [current_m, current_n]);
                workers_bad_rep_cdf = reshape(sim.PBTA_result{n, m}.dataObj.workers_bad_rep_cdf, [1, current_n]);
                ...
                %%PBTA MCMF reliability and reputaiton constraints dropped
                PBTA_MCMF_rel_rep_dropped_tasks_fail_prob = (PBTA_MCMF_rel_rep_dropped_x_reshaped .* joint_failure_probs) > failure_percentage;
                PBTA_MCMF_rel_rep_dropped_nodes_failed = sum(PBTA_MCMF_rel_rep_dropped_tasks_fail_prob, 2);
                PBTA_MCMF_rel_rep_dropped_actual_nodes = abs(PBTA_MCMF_rel_rep_dropped_nodes_failed - PBTA_MCMF_rel_rep_dropped_replicas);
                PBTA_MCMF_rel_rep_dropped_status = PBTA_MCMF_rel_rep_dropped_actual_nodes == 0;
                PBTA_MCMF_rel_rep_percentage = sum(PBTA_MCMF_rel_rep_dropped_status) ./ current_m;
                
                PBTA_MCMF_rel_rep_dropped_percentages_average(n, m) = PBTA_MCMF_rel_rep_dropped_percentages_average(n, m) + PBTA_MCMF_rel_rep_percentage;
                
                %%PBTA reliability and reputaiton constraints dropped
                PBTA_rel_rep_dropped_tasks_fail_prob = (PBTA_rel_rep_dropped_x_reshaped .* joint_failure_probs) > failure_percentage;
                PBTA_rel_rep_dropped_nodes_failed = sum(PBTA_rel_rep_dropped_tasks_fail_prob, 2);
                PBTA_rel_rep_dropped_actual_nodes = abs(PBTA_rel_rep_dropped_nodes_failed - PBTA_rel_rep_dropped_replicas);
                PBTA_rel_rep_dropped_status = PBTA_rel_rep_dropped_actual_nodes == 0;
                PBTA_rel_rep_dropped_percentage = sum(PBTA_rel_rep_dropped_status) ./ current_m;
                

                PBTA_rel_rep_dropped_percentages_average(n, m) = PBTA_rel_rep_dropped_percentages_average(n, m) + PBTA_rel_rep_dropped_percentage;
        ...
        %%PBTA MCMF reliability and reputation dropped
        PBTA_MCMF_rel_rep_dropped_percentages_average(n, m) = PBTA_MCMF_rel_rep_dropped_percentages_average(n, m) ./ number_of_simulations;
        %%PBTA reliability and reputation dropped
        PBTA_rel_rep_dropped_percentages_average(n, m) = PBTA_rel_rep_dropped_percentages_average(n, m) ./ number_of_simulations;
        %% Step 4: build graphs
        ...
        PBTA_MCMF_rel_rep_dropped_plot = strcat(PBTA_MCMF_rel_rep_dropped_plot, '(', num2str(current_m),', ', num2str(PBTA_MCMF_rel_rep_dropped_percentages_average(n, m)), ')');
        PBTA_rel_rep_dropped_plot = strcat(PBTA_rel_rep_dropped_plot, '(', num2str(current_m),', ', num2str(PBTA_rel_rep_dropped_percentages_average(n, m)), ')');
    ...
    disp('PBTA MCMF Plot: ');
    disp(PBTA_MCMF_plot);
    disp('PBTA MCMF rel rep dropped Plot: ');
    disp(PBTA_MCMF_rel_rep_dropped_plot);
    disp('PBTA reliability and reputation dropped Plot: ');
    disp(PBTA_rel_rep_dropped_plot);
