%% %% This script renders a graph that compares the percentage of failed nodes for the methods True, RepMax, PBTA, and MCMF
%Preparing the data
N_min = 100;
N_max = 100;
N_stepSize = 10;
...
dataObj.workers_history_percentageOfOnes_toval = 0.4;
...
for n=1:N
    for m=1:M
        ...
        for sim_count=1:number_of_simulations
            try 
                ...
                rep_max_total_failed_nodes = sum(rep_max_nodes_failed, 'all');
                ...
                rep_max_failed_nodes_percentage = rep_max_total_failed_nodes ./ sum(rep_max_replicas, 'all');
                rep_sw_percentage = sum(rep_sw_tasks_status) ./ current_m;
                rep_kw_percentage = sum(rep_kw_tasks_status) ./ current_m;

                rep_max_percentages_average(n, m) = rep_max_percentages_average(n, m) + rep_max_failed_nodes_percentage;
                ...
                if (enable_true_benchmark)
                    ...
                    TRUE_total_failed_nodes = sum(TRUE_nodes_failed, 'all');
                    ...
                    TRUE_failed_nodes_percentage = TRUE_total_failed_nodes ./ sum(true_replicas, 'all');
                end
                
                if (enable_true_benchmark)
                    true_percentages_average(n, m) = true_percentages_average(n, m) + TRUE_failed_nodes_percentage;
                end
                
                %%PBTA
                PBTA_tasks_fail_prob = (PBTA_x_reshaped .* joint_failure_probs) > failure_percentage;
                PBTA_nodes_failed = sum(PBTA_tasks_fail_prob, 2);
                PBTA_total_failed_nodes = sum(PBTA_nodes_failed, 'all');
                ...
                PBTA_failed_nodes_percentage = PBTA_total_failed_nodes ./ sum(PBTA_replicas, 'all');

                PBTA_percentages_average(n, m) = PBTA_percentages_average(n, m) + PBTA_failed_nodes_percentage;
                ...
                PBTA_MCMF_total_failed_nodes = sum(PBTA_MCMF_nodes_failed, 'all');
                ...
                PBTA_MCMF_failed_nodes_percentage = PBTA_MCMF_total_failed_nodes ./ sum(PBTA_MCMF_replicas, 'all');

                PBTA_MCMF_percentages_average(n, m) = PBTA_MCMF_percentages_average(n, m) + PBTA_MCMF_failed_nodes_percentage;
    ...
    disp(strcat('###Percentage of failed nodes, for M = ', num2str(current_m), 'and N = ', num2str(current_n), '###'));
    disp('RepMax Plot: ');
    disp(repMax_plot);
    if (enable_true_benchmark)
        disp('TRUE Plot: ');
